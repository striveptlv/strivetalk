...
onMounted(async () => {
  try {
    hasTalkAccess.value = await checkTalkAccess()
  } catch (error) {
    console.error('Talk access check failed:', error)
    hasTalkAccess.value = false
  } finally {
    isCheckingTalkAccess.value = false
  }
})
</script>
...
<template>
  <UApp class="min-h-screen">
    <main
      v-if="isCheckingTalkAccess"
      class="flex min-h-screen items-center justify-center bg-[#fffaf2] px-4 py-8 text-center text-[#0e2f5d] dark:bg-[#111113] dark:text-[#f4f4f5]"
    >
      <section>
        <img
        ...
        >
        <p class="mt-4 font-brand-heading text-2xl font-bold uppercase tracking-[0.08em] text-[#083d7a] dark:text-[#8ecae6]">
          {{ t("app.brand") }}
        </p>
    ...
    <template v-else-if="hasTalkAccess">
